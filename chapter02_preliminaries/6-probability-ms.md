...
from mindspore.ops import multinomial
...
```python
multinomial(fair_probs, 10)
```




    Tensor(shape=[10], dtype=Int32, value= [0, 4, 0, 5, 5, 2, 1, 2, 1, 5])
...
```python
# 将结果存储为32位浮点数以进行除法
counts = multinomial(fair_probs, 1000)
counts = np.bincount(counts, minlength=6)
...
```
    ...
    Tensor(shape=[6], dtype=Float32, value= [ 1.65999994e-01,  1.82999998e-01,  1.64000005e-01,  1.51999995e-01,  1.70000002e-01,  1.65000007e-01])
...
```
    ...
    Tensor(shape=[500, 10], dtype=Int64, value=
    [[1, 3, 4 ... 2, 2, 3],
     [4, 3, 2 ... 4, 4, 4],
     [1, 2, 3 ... 5, 0, 4],
     ...
...
```python
counts = ms.Tensor([np.bincount(multinomial(fair_probs, 10), minlength=6).asnumpy() for _ in range(500)])
