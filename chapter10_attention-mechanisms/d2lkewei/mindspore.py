...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    def init_weights(m):
        if type(m) == nn.Dense or type(m) == nn.Conv2d:
            m.weight.set_data(initializer(XavierUniform(), m.weight.shape))
            # 下面这段代码比较特殊，需要加深理解
            if m.has_bias:
                fan_in, _ = _calculate_fan_in_and_fan_out(m.weight.shape)
                bound = 1 / math.sqrt(fan_in)
                m.bias.set_data(initializer(Uniform(bound), [m.out_channels]))
    net.apply(init_weights)
    print('training on', ms.get_context('device_target'))
    ...
    grad_fn = ms.value_and_grad(forward_fn, None, weights=net.trainable_params(), has_aux=True)
...
def seq_data_iter_sequential(corpus, batch_size, num_steps):
    ...
    Xs = ms.Tensor(corpus[offset: offset + num_tokens])
    Ys = ms.Tensor(corpus[offset + 1: offset + 1 + num_tokens])
...
class RNNModelScratch(nn.Cell): #@save
    """从零开始实现的循环神经网络模型"""
    ...
    def construct(self, X, state):
        X = ops.one_hot(X.T, self.vocab_size, on_value=1, off_value=0).astype(ms.float32)
...
def predict_ch8(prefix, num_preds, net, vocab, device):  #@save
    ...
    get_input = lambda: ms.Tensor([outputs[-1]], ms.int32).reshape((1, 1)) #ms的nn.CrossEntropyLoss对类型检查过于严格，需要在此处强制类型转换
...
def train_epoch_ch8(net, train_iter, loss, updater, device, use_random_iter):
    ...
    def forward_fn(x, state, y):
        ...
        l = loss(y_hat, y.astype(ms.int32)).mean()
        return l
    grad_fn = ms.value_and_grad(forward_fn,grad_position=None,weights=net.trainable_params())
    net.set_train(True)
    for X, Y in train_iter:
...
class RNNModel(nn.Cell):
    ...
    def construct(self, inputs, state):
        X = ops.one_hot(inputs.T, self.vocab_size, on_value=1., off_value=0.)
        X = X.to(ms.float32)
...
def build_array_nmt(lines, vocab, num_steps):
    ...
    array = ms.Tensor([truncate_pad(
        l, num_steps, vocab['<pad>']) for l in lines])
    valid_len = (array != vocab['<pad>']).astype(ms.int32).sum(1)
...
def sequence_mask(X, valid_len, value=0):
    """在序列中屏蔽不相关的项"""
    maxlen = X.shape[1]
    mask = ops.arange((maxlen), dtype=ms.float32)[None, :] < valid_len[:, None]
    X[~mask] = value
    return X
...
class MaskedSoftmaxCELoss(nn.CrossEntropyLoss):
    ...
    def construct(self, pred, label, valid_len):
        weights = ops.ones_like(label, dtype=ms.int32)
        weights = sequence_mask(weights, valid_len)
        self.reduction='none'
        unweighted_loss = super(MaskedSoftmaxCELoss, self).construct(
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
    ...
    def xavier_init_weights(m):
        ...
        if type(m) == nn.GRU:
            init_ls = list([(name) for name, param in m.parameters_dict().items()])
            # print("Init", init_ls)
            init_dic = m.parameters_dict()
            # m.weight.set_data(initializer(Uniform(scale=10), m.weight.shape, m.weight.dtype))
            for param in init_ls:
                if "weight" in param:
                    init_dic[param].set_data(initializer(XavierUniform(),init_dic[param].shape,init_dic[param].dtype))
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            X, X_valid_len, Y, Y_valid_len = [x.astype(ms.int32) for x in batch]
            bos = ms.Tensor([tgt_vocab['<bos>']] * Y.shape[0]).reshape(-1, 1).astype(ms.int32)
            dec_input = ops.cat([bos, Y[:, :-1]], 1)  # 强制教学
            # Y_hat, _ = net(X, dec_input, X_valid_len)
            # l = loss(Y_hat, Y, Y_valid_len)
            # l.sum().backward()	# 损失函数的标量进行“反向传播”
            l, grads = grad_fn(X, dec_input, X_valid_len, Y, Y_valid_len)
            d2l.grad_clipping(grads, 1)
...
def predict_seq2seq(net, src_sentence, src_vocab, tgt_vocab, num_steps,
    ...
    src_tokens = src_vocab[src_sentence.lower().split(' ')] + [
        src_vocab['<eos>']]
    enc_valid_len = ms.Tensor([len(src_tokens)])
    src_tokens = d2l.truncate_pad(src_tokens, num_steps, src_vocab['<pad>'])
    # 添加批量轴
    enc_X = ops.unsqueeze(
        ms.Tensor(src_tokens, dtype=ms.int64), dim=0)
    enc_outputs = net.encoder(enc_X, enc_valid_len)
    dec_state = net.decoder.init_state(enc_outputs, enc_valid_len)
    # 添加批量轴
    dec_X = ops.unsqueeze(ms.Tensor(
        [tgt_vocab['<bos>']], dtype=ms.int64), dim=0)
    output_seq, attention_weight_seq = [], []
    for _ in range(num_steps):
        Y, dec_state = net.decoder(dec_X, dec_state)
        # 我们使用具有预测最高可能性的词元，作为解码器在下一时间步的输入
        dec_X = Y.argmax(axis=2)
        pred = dec_X.squeeze(axis=0).astype(ms.int32).item()
        # 保存注意力权重（稍后讨论）
...
def masked_softmax(X, valid_lens):
    ...
    if valid_lens is None:
        ...
        shape = X.shape
        if valid_lens.dim() == 1:
            valid_lens = ops.repeat_interleave(valid_lens, shape[1])
        else:
            valid_lens = valid_lens.reshape(-1)
        # 最后一轴上被掩蔽的元素使用一个非常大的负值替换，从而其softmax输出为0
        X = d2l.sequence_mask(X.reshape(-1, shape[-1]), valid_lens,
                              value=-1e6)
        return ops.softmax(X.reshape(shape), axis=-1)
...
class AdditiveAttention(nn.Cell):
    ...
    def construct(self, queries, keys, values, valid_lens):
        ...
        # 使用广播方式进行求和
        features = queries.unsqueeze(2) + keys.unsqueeze(1) #在第二维度和第一维度上增加维度
        features = ops.tanh(features)
...
class DotProductAttention(nn.Cell):
    ...
    def construct(self, queries, keys, values, valid_lens=None):
        ...
        scores = ops.bmm(queries, keys.transpose(0,2,1)) / math.sqrt(d)
        self.attention_weights = masked_softmax(scores, valid_lens)
        return ops.bmm(self.dropout(self.attention_weights), values)
...
class PositionalEncoding(nn.Cell):
    ...
    def __init__(self, num_hiddens, dropout, max_len=1000):
        ...
        self.dropout = nn.Dropout(p=dropout)
        # 创建一个足够长的P
        self.P = ops.zeros((1, max_len, num_hiddens))
        X = ops.arange(max_len, dtype=ms.float32).reshape(
            -1, 1) / ops.pow(10000, ops.arange(
            0, num_hiddens, 2, dtype=ms.float32) / num_hiddens)
        self.P[:, :, 0::2] = ops.sin(X)
        self.P[:, :, 1::2] = ops.cos(X)
...
def show_trace_2d(f, results):
    ...
    x1, x2 = ops.meshgrid(ops.arange(-5.5, 1.0, 0.1),
                          ops.arange(-3.0, 1.0, 0.1)).asnumpy()
...
def train_ch11(trainer_fn, states, hyperparams, data_iter,
    ...
    # Initialization
    # 初始化模型
    w = ms.Parameter(ops.normal(mean=0.0, stddev=0.01, shape=(feature_dim, 1)), name='w')
    b = ms.Parameter(ops.zeros((1)),name='b')
    net, loss = lambda X: d2l.linreg(X, w, b), d2l.squared_loss
    forward_fn = lambda X, y:loss(net(X), y).mean()
    grad_fn = ms.value_and_grad(forward_fn, grad_position=None, weights=[w,b])
    # 训练模型
    animator = d2l.Animator(xlabel='epoch', ylabel='loss',
